...
# Hobday-style fixed baseline (clipped to data-availability years inside the code)
BASELINE = (1982, 2024)

# Event definition (Hobday et al. 2016)
...
def open_sst(files_glob: str, roi: dict) -> tuple[xr.Dataset, str, str]:
    ...
    ds = xr.open_mfdataset(paths, combine="by_coords",
                           chunks={"time": CHTIME}, engine="netcdf4")
...
# --- Hobday detection core (vectorized, time-core) ---
def _detect_mask_1d(sst_1d: np.ndarray, thresh_1d: np.ndarray,
                    min_dur: int = MIN_DUR, max_gap: int = MAX_GAP) -> np.ndarray:
    """
    Hobday event mask on a single time series:
    exceed >= threshold; join gaps <= max_gap; drop runs < min_dur.
    """
...
# --- Per-grid climatology & threshold via Oliver's mhw.detect ---
def _clim_thresh_time_1d(ords_1d: np.ndarray, temp_1d: np.ndarray,
                         y0: int, y1: int, pct: int,
    ...
    Returns arrays with length == len(time).
    """
    ...
    # Oliver API
    _, clim = mhw.detect(
        ords_1d.astype(int), temp_1d.astype(float),
        climatologyPeriod=[int(y0), int(y1)],
        pctile=int(pct),
        minDuration=int(min_dur),
        joinAcrossGaps=True,
        maxGap=int(max_gap),
    )
...
def build_grid_baseline(ds: xr.Dataset, latn: str, lonn: str,
                        pctile: int = 90) -> tuple[xr.DataArray, xr.DataArray]:
    """
    Compute time-aligned climatology (seas_t) and threshold (thresh_t) **for every grid cell**
    using mhw.detect with Hobday settings. Returns (seas_t, thresh_t) with dims (time, lat, lon).
    """
    t_index = pd.to_datetime(ds["time"].values)
    y0 = max(BASELINE[0], t_index.year.min())
    y1 = min(BASELINE[1], t_index.year.max())
    ords_da = xr.DataArray(
        np.array([d.toordinal() for d in t_index], dtype=int),
        coords={"time": ds["time"]},
        dims=["time"],
    ).chunk({"time": -1})  # single time chunk for gufunc

    sst = ds[SST_VAR].chunk({"time": -1, latn: CHXY, lonn: CHXY})

    seas_t, thresh_t = xr.apply_ufunc(
        _clim_thresh_time_1d, ords_da, sst,
        input_core_dims=[["time"], ["time"]],
        output_core_dims=[["time"], ["time"]],
        vectorize=True, dask="parallelized",
        output_dtypes=[float, float],
        kwargs=dict(y0=int(y0), y1=int(y1), pct=int(pctile),
